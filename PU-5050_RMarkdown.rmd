...
You may wish to add a value calculated from your data somewhere within a sentence in your text. In this case code chunks would not be suitable, but you can use inline code. Instead of 3 back ticks you use only 1 before and afer r.

```{r echo=TRUE}
#The total number of people waiting in the hospital for appointments are `r 3+7`
```

The total number of people in the waiting room are `r 3+7`.
...
**Synthetic data sets** are a special type of fake data set that are created to keep the statistical properties. Synthetic datasets can be time consuming to create. If you are also doing research on a rare disease in a small population, it is likely the patient could be identified. So great care must be taken, but the benefits of fake data sets and synthetic data sets are essential for the reproducibility of health data.
...
If I was to give you a large dataset with no headings or any explanation of how the data was created, it would be useless. The better the metadata the more reproducible the overall project can be. The quicker the project can be reproduced and 
There are different standards for metadata that can be followed depending on which field you work or research. Information on different standards can be found [here.](https://fairsharing.org/standards/) 
...
A tidy dataset is when there is only one observation per row.
