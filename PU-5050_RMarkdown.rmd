...
We have named an object demographics. The used the read_csv function to 
...
```{r 3.4.10 Find the youngest person, echo=TRUE}
#Youngest person
demo_date%>%
...
#Using the function min() (minimum)
  summarise(min(dob))
```
```{r 3.4.11 Find the oldest person}
#Oldest person
demo_date%>%
...
```{r 3.4.12 Remove any dates beyond 2021-12-11}
...
```{r 3.3.13 demo_date_filter saved as a csv file}
...
```{r 3.3.14 Glimpse ImagingWaitTimes, echo=TRUE}
...
```{r 3.3.15 Summary of all the variables in WaitingDays, echo=TRUE}
...
```{r 3.3.16 Filiter for WaitingDays greater than zero and less than 365}
...
```{r 3.3.17 Check the maximum and minimum calues of WaitingDays, echo=TRUE}
...
```{r 3.3.18 Remove the missing values from ImagWaitTmesFilter, echo=TRUE}
ImagWaitTimesFilter%>%
  summarise_all(~sum(is.na(.)))%>%
   view()
...
```{r 3.3.19 remove the missing values from the ImagWaitTimeFilter dataset}
...
```{r 3.3.20 Check all the missing values have been removed, echo=TRUE}
WaitTime_Tidy%>%
summarise_all(~sum(is.na(.)))
``` 
***
...
```{r 3.3.21 save the object WaitTime_Tidy as a csv file}
...
```{r 3.3.22 use the inner_join function}
# The inner_join function will combine all the data in WaitTimeTidy with the relevant demo_date_filter observations with the same chi value.
...
```{r 3.3.23 inspect the join_demo_wait dataset, echo=TRUE}
...
ggplot(aes(fct_reorder(city,mean),mean))+
  ...
  # I gave the graph some colour to make it more visually pleasing. Initially, I just used coral as the colour, but sunglasses would have been required.
